...
def early_stopping_condition(best_logprobs, target_model, logprob_dict, target_token, determinstic_jailbreak, no_improvement_history=750, 
    ...
    if determinstic_jailbreak and logprob_dict != {}:
        ...
        if argmax_token in [target_token, ' '+target_token]:
            return True #достигнут целевой токен
        else:
            return False
    ...
    best_logprob = best_logprobs[-1] #Извлекает последнюю лучшую логарифмическую вероятность 
    if no_improvement_history < len(best_logprobs):
        ...
        no_sufficient_improvement_condition = prob_best - prob_history < no_improvement_threshold_prob # Пороговое значение улучшения вероятности.
    else: 
